...
uint16_t ECH210BModbusMaster232::_crc16_update2(uint16_t crc, uint8_t a)
{
  int i;
...
/**
  ...
  @overload voidModbusMaster::ModbusMaster(uint8_t u8MBSlave)
  @param u8RxPin rx pin
  @param u8TxPin tx pin
  @param u8MBSlave Modbus slave ID (1..255)
  @ingroup setup
*/
void ECH210BModbusMaster232::init(uint8_t u8RxPin, uint8_t u8TxPin, uint8_t u8MBSlave)
{
  // define pin modes for tx, rx:
  pinMode(u8RxPin, INPUT);
  pinMode(u8TxPin, OUTPUT);
  pSwSerial = new SoftwareSerial8E1(u8RxPin, u8TxPin);
...
/**
  ...
  @ingroup setup
*/
void ECH210BModbusMaster232::begin(unsigned long BaudRate)
{
...
/**
  ...
  @ingroup register
*/
uint8_t ECH210BModbusMaster232::readHoldingRegisters(uint16_t u16ReadAddress,
                                                     uint16_t u16ReadQty)
{
...
/**
  ...
  @ingroup buffer
*/
uint16_t ECH210BModbusMaster232::getResponseBuffer(uint8_t u8Index)
{
...
/**
  ...
  @ingroup buffer
*/
void ECH210BModbusMaster232::clearResponseBuffer()
{
  uint8_t i;
...
/**
  ...
  @return 0 on success; exception number on failure
*/
uint8_t ECH210BModbusMaster232::ModbusMasterTransaction(uint8_t u8MBFunction)
{
  ...
  for (i = 0; i < u8ModbusADUSize; i++)
  {
    ...
    DEBUG_MOD_PRINT("_");
    pSwSerial->print(char(u8ModbusADU[i]));
  }
  DEBUG_MOD_PRINTLN("");
  ...
  {
    if (pSwSerial->available())
    {

      u8ModbusADU[u8ModbusADUSize] = pSwSerial->read();
